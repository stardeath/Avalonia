...
{
    internal class GtkStorageProvider : IStorageProvider
    {
            ...
            IEnumerable<FilePickerFileType>? filters, string? defaultExtension, bool overwritePrompt)
        {
            ...
            if (initialFolder?.TryGetFullPath(out var folderPath) == true)
            {
                initialFileName = initialFileName is null
                    ? folderPath
                    : Path.Combine(folderPath, initialFileName);
            }
